...
def gcd_of_last_digits(int_one, int_two)
    x = int_one.last_digit
    y = int_two.last_digit
    return (x).gcd(y)
end
...
def dec_remainder_of_two_floats(f_dividend, f_divisor)
    return ((f_dividend / f_divisor).floor - (f_dividend / f_divisor)).abs.round(1)
end

# Write a method that returns the decimal remainder of dividing two integers.
# HINT: Use dec_remainder_of_two_floats as a hel%per method,
...
def int_remainder_without_modulo(i_dividend, i_divisor)
    return i_dividend - (i_dividend.to_f/i_divisor.to_f).floor * (i_divisor) 
end